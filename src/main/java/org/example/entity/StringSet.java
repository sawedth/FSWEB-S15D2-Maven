...
public class StringSet {
    ...
    public static Set<String> findUniqueWords() {
        String modified = text.replaceAll("\\p{Punct}", "");
        modified = modified.replaceAll("[0-9]", "");
        modified = modified.toLowerCase();
        String[] modifiedArr = modified.split(" ");
        Set<String> modifiedSet = new TreeSet<>(List.of(modifiedArr));
        modifiedSet.remove("");
        System.out.println(modifiedSet.size());
